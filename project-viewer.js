...
function create() {
  ...
  dialog.setAttribute("aria-label", t("pvViewer"));
  // Static interface only. Captions and image URLs are assigned as properties.
  dialog.innerHTML = `<button type="button" class="pv-close" aria-label="${t("pvClose")}">×</button><div class="pv-stage"><img alt=""></div><div class="pv-controls"><button type="button" class="pv-prev" aria-label="${t("pvPrev")}">←</button><span class="pv-count" aria-live="polite"></span><a class="pv-full-size" target="_blank" rel="noopener">${t("pvFull")}</a><button type="button" class="pv-next" aria-label="${t("pvNext")}">→</button></div><p class="pv-caption" role="status"></p>`;
...
async function show(direction = 0) {
  ...
  try {
    ...
    if (token === current && dialog.open) caption.textContent += t("pvPreviewNote");
