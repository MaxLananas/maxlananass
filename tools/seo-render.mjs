...
const imageFiles = (page) => page.galleryPage
  ...
  : (page.project?.images || []).map((name) => FILES.find((item) => item.name === name));
...
export function schemaFor(page, pages, h) {
  ...
  const person = {
    ...
    knowsAbout: ["Minecraft building", "BuildTheEarth", "Java development", "Web development", "Software interfaces"],
  ...
  if (page.type === "Article") {
    const article = { "@type": "Article", "@id": url + "#article", headline: page.heading, description: page.description,
      ...
      citation: PROJECTS.filter((p) => p.kind === "software").map((p) => p.repo) };
    graph.push(article);
    webPage.mainEntity = { "@id": article["@id"] };
  }
  if (page.project) {
    const p = page.project;
    const work = { "@id": url + "#project", "@type": p.kind === "software" ? "SoftwareSourceCode" : "CreativeWork",
    ...
    if (p.repo) {
      ...
      work.citation = p.repo + "/blob/" + p.revision + "/" + p.evidenceFile;
    }
    if (p.bte) work.about = { "@id": canonical("/buildtheearth/#initiative") };
    if (p.kind === "software") work.programmingLanguage = p.language;
  ...
  if (photos.length) {
    for (const item of photos) {
      ...
      graph.push({ "@type": "ImageObject", "@id": url + `#image-${index + 1}`, contentUrl: canonical(h.imageData(item).src),
        name: imageLabel(item, index), caption: imageLabel(item, index),
        creditText: credit ? `${credit.text} ${credit.linkText}` : "From MaxLananas’s personal build portfolio",
        isPartOf: { "@id": webPage["@id"] } });
    }
    webPage.hasPart = photos.map((item) => ({ "@id": url + `#image-${FILES.indexOf(item) + 1}` }));
  }
  const selected = page.path === "/projects/" ? PROJECTS : page.path === "/development/" ? PROJECTS.filter((p) => p.kind === "software") : page.path === "/buildtheearth/" ? PROJECTS.filter((p) => p.bte) : [];
  if (selected.length) {
...
${meta("og:image", canonical("/assets/social/portfolio.png"), true)}
${meta("og:image:type", "image/png", true)}
${meta("og:image:width", "1200", true)}
${meta("og:image:height", "630", true)}
${meta("og:image:alt", "MaxLananas — Minecraft builds and developer projects", true)}
${meta("twitter:card", "summary_large_image")}
${meta("twitter:title", page.title)}
${meta("twitter:description", page.description)}
${meta("twitter:image", canonical("/assets/social/portfolio.png"))}
${meta("twitter:image:alt", "MaxLananas — Minecraft builds and developer projects")}
...
export async function renderSeo({ manifest = {}, base = "/", ...custom } = {}) {
  ...
  for (const page of pages) {
    ...
    const html = page.home ? template.replace("{{SEO_HEAD}}", head).replace("{{HOME_INTRO}}", homeIntro(page, h))
      .replace("{{HOME_PROJECTS}}", homeProjects(h)).replaceAll("{{IMAGE_COUNT}}", String(FILES.length)).replaceAll("{{BASE}}", base)
  ...
  files.set("sitemap-images.xml", `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:image="http://www.google.com/schemas/sitemap-image/1.1">\n${indexable.filter((p) => imageFiles(p).length).map((p) => `  <url><loc>${escape(canonical(p.path))}</loc>${imageFiles(p).map((item) => `<image:image><image:loc>${escape(canonical(h.imageData(item).src))}</image:loc></image:image>`).join("")}</url>`).join("\n")}\n</urlset>\n`);
