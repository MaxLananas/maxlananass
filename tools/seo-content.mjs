...
function projectPage(page, h) {
  ...
  const sourceLabel = p.modrinth ? t("sourceModrinth") : l === "en" ? p.source?.label || t("sourceRepo") : t("sourceRepo");
  ...
  const shots = p.images ? `<section><h2>${t("sectionShots")}</h2>${h.photos(p.images.map((name) => FILES.find((file) => file.name === name)))}</section>` : "";
