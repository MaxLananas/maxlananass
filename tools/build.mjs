...
export async function buildSite({ outDir = "dist", sourceDir, offline = false, imageOptions = {}, base = "/", quiet = false } = {}) {
  ...
  const result = await build({
    absWorkingDir: root,
    entryPoints: { app: "script.js", styles: "style.css", "not-found": "404.css" },
  ...
  html = html
    ...
    .replace(/<link rel="preconnect" href="https:\/\/wsrv.nl"[^>]*>\n/g, "")
    .replace('href="style.css"', `href="${styles}"`)
    .replace('href="assets/fonts/FFFlauta-200.woff2"', `href="${font}"`)
    .replace('<script type="module" src="script.js"></script>', preloads.map((href) => `<link rel="modulepreload" href="${href}">`).join("\n") + `\n<script type="module" src="${entry}"></script>`);
  ...
  const staticFiles = ["CNAME", "robots.txt", "sitemap.xml", "manifest.json", "apple-touch-icon.png"];
  ...
  const assets = [...outputEntries.map(([file]) => outputURL(file)), ...creditFiles, "index.html", "404.html", "apple-touch-icon.png", "manifest.json"].sort();
  const versionHash = createHash("sha256");
  for (const asset of assets) { versionHash.update(asset); versionHash.update(await readFile(join(outDir, asset))); }
  const version = versionHash.digest("hex").slice(0, 16);
  const workerSource = (await readFile(join(root, "sw.js"), "utf8"))
    .replace('const STATIC_VERSION = "source-v7";', `const STATIC_VERSION = "${version}";`)
  ...
  const summary = { ...report, build: version, code };
